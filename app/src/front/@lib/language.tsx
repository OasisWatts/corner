...
export default class L {
      ...
      private static parse(value: string, ...args: any[]): React.ReactNode {
            ...
            value = value.replace(L.REGEXP_STRICT_ARGS, (p, v1) => args[v1])
                  .replace(L.REGEXP_ARGS, (p, v1) => {
                        blockBank.push(args[v1])
                        return "<{__}>"
                  })
            while (execArray = PATTERN.exec(value)) { // regexp 한걸 다시 배열
...
export function getTimeDistance(from: number, to: number = Date.now()) {
      return (to - from) / UnitDate.MINUTE
}
/**
